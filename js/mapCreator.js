...
function paint(ctx)
{
	...
	for(var i=0;i<map.h;i++)
	{
		...
		{	
			...
			if (map.get(j,i)===1)ctx.fillStyle='#977';
			else
			if (map.get(j,i)===2)ctx.fillStyle='#0f0';
			else
			if (map.get(j,i)===3)ctx.fillStyle='#f00';
...
window.addEventListener("mousemove",function mouseMove(evt)
{
	...
	var xcorrection=0.5,ycorrection=0.5;
...
window.addEventListener("mousedown",function mouseClick(evt)
{
    mouseDown(evt);
	mouseFlag = 1;
},false);
...
{
    if (evt.keyCode==83 && !map.isStart()) //S
	{
